...
def snapshot_metrics(
    ...
    *,
    ...
    metrics = [
        ...
        measured("processed_frame_count", "count", "frame", snapshot.total_frames, case_ids, "must_be_positive", "frames processed in measurement window", threshold_result="PASS" if snapshot.total_frames > 0 else "FAIL"),
        measured("dropped_capture_frame_count", "count", "frame", snapshot.dropped_capture_frames if case.source_kind == "webcam" else None, case_ids, "observation_only", "capacity-one webcam mailbox overwrites; not applicable to sequential video", status="MEASURED" if case.source_kind == "webcam" else "NOT_APPLICABLE"),
        measured("degraded_frame_rate", "single", "ratio", snapshot.degraded_frames / snapshot.total_frames if snapshot.total_frames else None, case_ids, "observation_only", "degraded frames / processed frames"),
        measured("retained_source_read_to_render_sample_count", "count", "count", snapshot.retained_source_read_to_render_samples, case_ids, "no_truncation", "retained bounded samples", threshold_result="PASS" if snapshot.retained_source_read_to_render_samples == snapshot.total_frames else "FAIL"),
        measured("retained_source_read_to_display_submit_sample_count", "count", "count", snapshot.retained_source_read_to_display_submit_samples, case_ids, "no_truncation_gui", "GUI retains one display sample per frame; headless retains zero", threshold_result="PASS" if (snapshot.retained_source_read_to_display_submit_samples == snapshot.total_frames if case.display else snapshot.retained_source_read_to_display_submit_samples == 0) else "FAIL"),
    ]
    if case.display:
        metrics.extend(
            [
